# coding: utf-8
'''
This tool is from https://github.com/wangshub/wechat_jump_game/blob/master/wechat_jump_auto.py
Author: wangshub

In my project, it is used to mark the place of the objects, 
so that a picture can produce more samples for feeding the 
CNN. 

The purpose of using it is to save time. In theory, it is 
the same as playing longer without this.
'''
import os
import sys
import subprocess
...
def jump(distance):
    ...
    press_time = int(press_time+random.randrange(-80,80))
    #x = swipe_x1+random.randrange(-200,200)
    #y = swipe_y1+random.randrange(-200,200)
    cmd = 'adb shell input swipe {x1} {y1} {x2} {y2} {duration}'.format(
        x1=swipe_x1+random.randrange(-200,200),
        y1=swipe_y1+random.randrange(-200,200),
        x2=swipe_x1+random.randrange(-200,200),
        y2=swipe_y1+random.randrange(-200,200),
        duration=press_time
    )
    print(cmd)
    os.system(cmd)
