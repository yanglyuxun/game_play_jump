...
# -*- coding: utf-8 -*-
"""
This is a fast sampling version
for each image, use a sample tool to mask 
and give all results of actions
...
import sample_tool as st

def calc_ms(img):
    'input img and calc the best ms number'
    piece_x, piece_y, board_x, board_y = st.find_piece_and_board(img)
    d = np.sqrt((board_x - piece_x) ** 2 + (board_y - piece_y) ** 2)
    press_time = d * st.press_coefficient
    press_time = max(press_time, 200)   # 设置 200ms 是最小的按压时间
    return int(press_time)

def get_ra_t(img,maps):
    'input img(colorful) and output the all rewards for actions'
    t = calc_ms(img)
    step = maps[1]-maps[0]
    a = []
    for i in maps:
...
def restart():
    ...
    top = int(np.random.uniform(top-10, top+10))
    d.swipe(left,top,left,top, np.random.choice(3))
...
class jump_API(object):
    ...
    def next_step(self,action, wait = 1):
        ...
        img1 = screen_shot()
        die = has_die(img1[1])
        ra_t = get_ra_t(self.img[0],self.a_map)
        if die:
            r_t = 0
            ...
            self.img = img1
            self.s_t = get_state(self.img[1])
        return self.s_t.copy(), r_t, die, ra_t.copy()
